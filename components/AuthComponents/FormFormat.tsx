...
function FormFormat({ title, description, accent, children }: FormFormatProps) {
  return (
    <Card className="w-full max-w-lg bg-[var(--theme-background)] border-none shadow-none">
      <CardHeader className="flex flex-col items-center mb-4 w-full">
        ...
        />
        <CardTitle className="flex flex-row items-baseline whitespace-nowrap text-5xl text-[var(--theme-text)]">
          <span>{title}</span>
        ...
        {description && (
          <CardDescription className="text-[var(--theme-textSecondary)]">
            {" "}
            {description}
          </CardDescription>
        )}
